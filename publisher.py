...
client = mqtt.Client(CallbackAPIVersion.VERSION2)   # version1 take 4 args, while version2 take 5 args in callback
...
client.loop_start()     # thread
# client.loop_forever() # blocked

import time

for i in range(10):
    client.publish(topic='/lock', payload=i)
    print(f"send{i}")
    time.sleep(1)
